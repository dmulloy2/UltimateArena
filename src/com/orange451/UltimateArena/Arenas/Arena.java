...
public abstract class Arena {
	...
	public void announce() {
		if (announced == 0) {
			az.plugin.SendMessageAll(ChatColor.AQUA + az.arenaType + ChatColor.GOLD + " arena has been created!");
		}else{
			az.plugin.SendMessageAll(ChatColor.GOLD + "Hurry up and join the " + ChatColor.AQUA + az.arenaType + ChatColor.GOLD + " arena!");
		}
		az.plugin.SendMessageAll(ChatColor.GOLD + "type " + ChatColor.AQUA + "/ua join " + az.arenaName + ChatColor.GOLD + " to join!");
		announced++;
	}
	...
	public void spawnAll() {
		System.out.println("[ULTIMATEARENA] SPAWNING ALL PLAYERS");
		az.plugin.getServer().getScheduler().scheduleSyncDelayedTask(az.plugin, new Runnable() {
		    public void run() {
				//spawns every player
				for (int i = 0; i < arenaplayers.size(); i++) {
					try{
			...
			if (pl != null) {
				...
				if (ap.killstreak == 5) {
					if (!(this.az.arenaType.equalsIgnoreCase("cq"))) {
						...
						for (int i = 0; i < 4; i++) {
							pl.getLocation().getWorld().spawnCreature(pl.getLocation(), EntityType.ZOMBIE);
						}
					}
				}
				if (ap.killstreak == 8) {
	...
	public void onDisable() {
		tellPlayers("This arena has been disabled!");
		this.gametimer = -1;
		disabled = true;
		stop();
	}
	...
	public void stop() {
		...
		stopped = true;
		onStop();
		System.out.println("[ULTIMATEARENA] PREPARING TO STOP ARENA " + name);
		//az.plugin.getServer().getScheduler().cancelTask(timer);
		try{
			for (int i = 0; i < arenaplayers.size(); i++) {
				try{
					...
					if (player != null) {
						if (this.az.plugin.isInArena(player)) {
							if (gametimer <= maxgametime) {
								Util.MatchPlayer(arenaplayers.get(i).player.getName()).sendMessage("Game inturrupted/ended!");
							}else{
								Util.MatchPlayer(arenaplayers.get(i).player.getName()).sendMessage("Game Over!");
							}
				...
				try{
					if (!ap.out) {
						...
						if (!(az.plugin.isInArena(pl.getLocation()))) {
							System.out.println("[ULTIMATEARENA] " + ap.player.getName() + " GOT OUT OF THE ARENA! putting him back in");
							ap.spawn();
							spawn(ap.player.getName(), false);
						}
					}
	...
	public void endPlayer(final ArenaPlayer p, boolean dead) {
		...
		try{
			...
			if (pl != null) {
				...
				updatedTeams = true;
				if (dead) {
					pl.sendMessage("You have exceeded the death limit!");
				}
			}
		}catch(Exception e) {
			e.printStackTrace();
